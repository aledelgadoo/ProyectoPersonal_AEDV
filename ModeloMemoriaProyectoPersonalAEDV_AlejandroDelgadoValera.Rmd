...
.gm-plot, .plotly, .html-widget, .figure, .ggplot {
  font-family: "Georgia", serif;
  font-size: 14px;
  color: #222;
}
...
```{r setup-ggplot-theme, include=FALSE}
library(ggplot2)
...
theme_set(
  theme_minimal(base_family = "Georgia") + 
    theme(
      ...
      plot.subtitle = element_text(face = "italic", size = 14, hjust = 0.5, color = "#444444"),
      
      # Ejes
      axis.title = element_text(face = "bold", size = 14, color = "#222222"),
      axis.text = element_text(size = 12, color = "#222222"),
      
      # Leyenda
      legend.title = element_text(face = "bold", size = 13, color = "#222222"),
...
tb_top %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
    ...
    theme_minimal(base_size = 14) + # Aplica el estilo  y ajusta el tamaño de la tipografía
    theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1)) + # Quita la leyenda y pone los nombres 45º
    annotate("text", x = length(unique(tb_bottom$full_name)) + 1.2, 
...
tb_bottom %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
    ...
    theme_minimal(base_size = 14) + # Aplica el estilo  y ajusta el tamaño de la tipografía
    theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1)) + # Quita la leyenda y pone los nombres 45º
    annotate("text", x = length(unique(tb_bottom$full_name)) + 0.5, 
             y = valor_españa, label = "Spain", color = "red", vjust = -0.5, hjust = 1.1, size = 4)
...
tb_top %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
    ...
    geom_bar(stat = "identity") +
    labs(x="",y="Paises con un mayor coste laboral por hora trabajada") + 
...
```{r, results=F}
tb_top_evol <- data %>% 
    filter(full_name %in% tb_top$full_name, NUTS==0, item=="D1_SAL_HW", unit=="EUR", year>=2000) %>%
...
tb_top_evol %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
    ...
    geom_line() +
    labs(x="",y="Evolución del coste por hora trabajada") +
...
Compararemos ahora la remuneración por hora trabajada según el sector en los últimos 5 años.
```{r, fig.width=10}
tb <- data %>% 
...
```
Vamos a relacionar ahora el precio por hora trabajada con los países más releventes en este aspecto.
...
top_countries %>% 
    ggplot(aes(reorder(full_name, -mean_value),sector_name,fill=mean_value)) +
    ...
    geom_text(aes(label = round(mean_value,1)), color = "black") +
    labs(x="",y="") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
