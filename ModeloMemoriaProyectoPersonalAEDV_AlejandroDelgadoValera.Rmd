...
tb_top_evol <- data %>% 
    filter(full_name %in% seleccion_paises, NUTS==0, item=="D1_SAL_HW", unit=="EUR", year>=2000) %>%
...
tb_top_evol %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
...
```{r top-bottom-carga-laboral, fig.width=8, fig.height=5.5}
tb <- data %>% 
...
tb_top %>% mutate(full_name = factor(full_name, levels=(unique(full_name)))) %>% 
    ...
    labs(x="",y="Precio por hora trabajada (€)", title = "Paises con un mayor coste laboral por hora trabajada") + 
...
```{r heatmap-sector-paises, fig.width=10}
...
```{r heatmap-sector-comunidades, fig.width=12.5, fig.height=7.5}
...
```{r precio-hora-por-sector, fig.width=10}
tb <- data %>% 
...
```{r coste-sector-espana-2023, fig.height=5, fig.width=7.5}
tb <- data %>%
...
Para entender la variabilidad interna, se incluye un boxplot del coste laboral medio por persona empleada. Este gráfico permite observar los valores típicos, la dispersión y posibles valores atípicos dentro de los distintos sectores y regiones.
```{r boxplot-coste-persona}
seleccion_paises <- c("Austria", "Belgium", "Denmark", "Finland", "France", "Germany", "Ireland", "Italy", "Luxembourg", "Netherlands", "Spain", "Sweden")
...
tb %>%
    ...
    geom_boxplot() +
    labs(x="", y="Coste anual (€)", title = "Coste laboral medio por persona empleada") +
...
horas.españa.omit.covid.prediccion %>% 
    autoplot(horas.españa) +
    labs(title = "Horas trabajadas anuales en ES - Predicción futura", x = "Año", y = "Horas")
```
...
```{r evolucion-remun-españa-data, results = F}
remun.españa <- data %>% 
...
remun.españa
```
```{r evolucion-remun-españa}
remun.españa %>% 
    autoplot() +
    labs(title = "Evolución del coste laboral por hora trabajada en España", x="Año", y="Euros / Hora")
```
...
```{r evolucion-remun-españa-canarias}
...
comparacion.ES_IC.remun %>% rename(Región = geo) %>% 
    autoplot() +
    labs(title = "Coste laboral por hora trabajada - ES vs IC", x="Año",y="Euros / Hora") +
    scale_color_manual(values = c("Canarias" = "steelblue", "España" = "red"))
```
...
```{r evolucion-remun-españa-europa}
...
```{r evolucion-remun-españa-arima, results = F}
remun.españa.arima <- remun.españa %>% 
    model(ARIMA(values))
```
```{r descomposicion-remun-españa}
...
```{r evolucion-remun-españa-prediccion}
remun.españa.prediccion <- remun.españa.arima %>%
     forecast(h="5 years")

remun.españa.prediccion %>% 
    autoplot(remun.españa) +
    labs(title = "Coste laboral por hora trabajada en ES - Predicción futura", x = "Año", y = "Euros / Hora")
```

```{r real-vs-arima-remun, fig.width=8}
# Generamos el data frame con los valores reales y ajustados del modelo
